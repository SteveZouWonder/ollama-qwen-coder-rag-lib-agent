...
**最后更新**: 2026-09-09（F10 P3-1 完成：Tesseract 三平台自动探测与缺失提示（F5 残留小项并入并移除）、README 1448 → 323 行迁入 docs/tutorials 并新增 08 / 09；此前 P2-1 完成：`bm25_store` 持久化增量、混合检索关闭可见、注册中心 / 调度器 RLock、`OLLAMA_MAX_CONCURRENCY` 并发限流与子任务排队不计超时）
