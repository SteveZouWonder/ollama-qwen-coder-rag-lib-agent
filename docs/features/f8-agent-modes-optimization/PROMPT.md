...
> 状态：F8 P0-P4 已全部完成（2026-09-07），本文件作为**流程模板**保留——后续同类任务（新 P 级 / 其他功能）
> 复制「提示词正文」并替换范围与 REQ 路径即可。
>
...
先完整阅读 docs/features/f8-agent-modes-optimization/REQUIREMENTS.md（REQ）。其 §0 是立项时核实的代码事实与文件:行号（§0.4 为 P4 分块相关），直接采信，勿重复调研；§1-§5 是 P0-P4 需求与验收；§6 实施顺序；附录 A 是 LLM 提示词草案。同目录 README.md 是实现记录，已完成的 P 级以它为准。
...
- 每个 P 级完成：全量测试通过 → 更新 CHANGELOG.md [Unreleased]（新增/改进/修复）、主 README 对应段落、同目录 README.md（状态表 + 实现记录 + 差异 + 验证记录）、REQ 顶部状态行、docs/features/README.md 与 ROADMAP.md → 中文 commit（风格见 git log -5）→ 询问是否创建 PR（不得自动创建）。
...
## 设计说明（为什么这样写）
- 需求（REQ）与实现记录（README）分文件：REQ 只增不改、保持"当时为什么这么定"；实现差异、验证结果集中在 README，避免 REQ 头部堆叠多轮记录。
