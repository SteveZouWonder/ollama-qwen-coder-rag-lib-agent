...
| 项 | 结论 |
|---|---|
| 工具/命令面板 | ✅ 已实现（「工具」页：代码助手（解释 / 审查 / 测试 / 文档 / 重构，受限只读 Agent 流式 + 符号表 / 质量卡片）/ Git 仪表盘 / 数据库（表列表 · Schema · 自然语言 → SQL → 确认运行 · 结果表格）/ 工作区（目录浏览 · 语法高亮分页预览 · 搜索 · 编辑 · 自然语言 → 命令 → 风险分析 → 执行），每个结果可「用 AI 解读」或「发送到对话」；Git 子页含 暂存预览 → AI 生成提交信息（可编辑）→ 确认提交；数据库连接下拉记住最近 8 个库、命令区「历史命令」下拉记住最近 50 条（`.cerebro/web_tools_state.json`）；表格空态一行说明、AI 任务运行期锁按钮；网络搜索子页已移除，缓存管理迁到「系统 → 运行环境」）。全部由 [F9](../f9-web-tools-revamp/README.md) 交付 |
