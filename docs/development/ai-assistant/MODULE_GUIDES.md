...
### `query_interface.py` / `cli/`（F10 P2-2 拆包，P3-2 二次拆分）
- `cli/state.py`：进程内共享状态——`HAS_RICH / HAS_READLINE / HAS_PROMPT_TOOLKIT` 探测、`get_console()` / `console`、`rag_engine / react_engine / last_rag_sources / last_web_sources / command_recommender`（由 `query_interface.main` 装配）、`_progress_state`。**其他模块一律 `from cli import state` 后在函数体内 `state.console` 运行时取值**，不要 `from cli.state import console`（导入时固化对象，patch 失效）。
- `cli/parser.py`：`parse_command()` → `ParsedCommand(cmd_type, raw, arg)`、`classify_mode()`（纯函数）；`query_interface` 重导出三者。
- `cli/help_text.py`：`TUTORIAL_TEXT`、`print_help(console, has_rich)`（`/help` 文案内联在函数体内，`inspect.getsource` 可校验）；`cli.render.print_help()` 为零参包装（读 `state.console / state.HAS_RICH`）。
- `cli/render.py`：`CEREBRO_ASCII / print_banner / backend_banner_text`、`show_tutorial / check_first_run`、`print_tools`、`print_rag_sources / _source_code_location / count_code_sources`、`print_knowledge_stats`、`_render_meta_overview`、`print_web_sources`、`_render_answer / _live_answer / _print_notices / _citation_summary`。
- `cli/callbacks.py`：`STEP_PHASE_EMOJI / STEP_PHASE_COLOR`、`_live_streaming`、`on_step_callback`（读写 `state._progress_state`）、`on_confirm_callback`、`ask_progress_callback`。
- `cli/rag_adapter.py`：`rag_pipeline` 的 10 个别名（`_is_meta_query / _parse_web_sources / _format_kb_context` …）、`run_web_search / enrich_with_page_content` 薄封装、`_cli_ask_progress`（结构化进度事件 → Rich 终端）、`_answer_meta_query`、`_synthesize_prompt`（CLI 版分区 prompt；P3-2 删除了被它覆盖的同名别名行）。
- `cli/recommend.py`：`show_command_recommendations / record_command_execution / record_conversation`、`_conversation / _print_health_hint / _health_before`。
- `cli/engine_commands.py`：引擎耦合命令 `handle_clear / history / summary / reset / file / write / exec / pwd / cd / model / think / auto / ask / agent / natural / unknown_cmd`（签名 `(ctx, parsed)`，直接读写 `state.rag_engine / state.react_engine / state.last_*_sources`）、`_run_ask / _ingest_inline_file / _augment_with_web_search / _answer_question / _route_natural_to_agent`、`_ENGINE_HANDLERS`、`_build_cli_context()`（每次分发从 `cli.state` 装配新 `CLIContext`）、`dispatch_command(ctx, parsed)`（先 `_ENGINE_HANDLERS` 再 `cli.handlers.COMMAND_HANDLERS`）、`KNOWLEDGE_MANAGEMENT_AVAILABLE`。
...
- `query_interface.py`（563 行）只保留：解释器自保护（`find_venv_python / ensure_compatible_interpreter`）、`setup_logging`、`setup_readline / get_input`、`main`（argparse、引导、引擎装配写入 `cli.state`、单次 `--query / --agent / --build-only`、REPL）；顶部重导出 `cli.*` 全部公开名并保留 `HAS_RICH / console / rag_engine` 等**导入时快照**别名（只供旧导入路径，函数体内不用）。
- 测试打桩：引擎 / 控制台 / 探测 → `patch("cli.state.console")` / `patch("cli.state.rag_engine", …)`；协作函数 → 调用方模块（`patch("cli.engine_commands.record_command_execution")`、`monkeypatch.setattr("cli.engine_commands._health_before", …)`）；`conftest.reset_module_state` 重置 `cli.state._progress_state / rag_engine`。**打桩 handler 内部依赖须以实际子模块为目标**（如 `cli.handlers.git._git_overview`、`cli.handlers.db._db_results`、`cli.handlers.session._get_session_manager`），patch `query_interface.X` / `cli_handlers.X` 不再生效。
