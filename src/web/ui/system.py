...
| 🧰 工具 | 代码助手（解释 / 审查 / 生成测试 / 生成文档 / 重构建议，附符号表与质量卡片）、Git 仪表盘（分支 / 变更 / 提交 / 作者）与 AI 提交信息、SQLite 连接 / 表结构 / 自然语言生成 SQL → 确认运行 / 结果表格、工作区目录浏览 / 文件预览与编辑 / 自然语言生成命令 → 风险分析 → 执行；每个结果可「用 AI 解读」或「发送到对话」 | `/agent` `/code-*` `/git-*` `/db-*` `/exec` `/file` `/write` |
